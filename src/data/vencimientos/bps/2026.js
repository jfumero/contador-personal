const vencimientosBPS2026 = {
    cobranzaDescentralizada: {
        nombre: "Cobranza Descentralizada",
        descripcion:
        "Aportes BPS de Industria y Comercio.",

        meses: [
            {
                mes: "Enero",
                periodo: "01/2026",
                vencimiento: "2026-01-23",
            },

            {
                mes: "Febrero",
                periodo: "02/2026",
                vencimiento: "2026-02-24",
            },

            {
                mes: "Marzo",
                periodo: "03/2026",
                vencimiento: "2026-03-20",
            },

            {
                mes: "Abril",
                periodo: "04/2026",
                vencimiento: "2026-04-24",
            },

            {
                mes: "Mayo",
                periodo: "05/2026",
                vencimiento: "2026-05-25",
            },

            {
                mes: "Junio",
                periodo: "06/2026",
                vencimiento: "2026-06-22",
            },

            {
                mes: "Julio",
                periodo: "07/2026",
                vencimiento: "2026-07-21",
            },

            {
                mes: "Agosto",
                periodo: "08/2026",
                vencimiento: "2026-08-21",
            },

            {
                mes: "Setiembre",
                periodo: "09/2026",
                vencimiento: "2026-09-21",
            },

            {
                mes: "Octubre",
                periodo: "10/2026",
                vencimiento: "2026-10-22",
            },

            {
                mes: "Noviembre",
                periodo: "11/2026",
                vencimiento: "2026-11-23",
            },

            {
                mes: "Diciembre",
                periodo: "12/2026",
                vencimiento: "2026-12-21",
            },
        ],
    },

    snisServiciosPersonales: {
        nombre: "SNS / FONASA",
        descripcion:
        "Pago SNS Servicios Personales. Prestador: CAAMEPA.",

        meses: [
            {
                mes: "Enero",
                periodo: "01/2026",
                vencimiento: "2026-01-26",
            },

            {
                mes: "Febrero",
                periodo: "02/2026",
                vencimiento: "2026-02-24",
            },

            {
                mes: "Marzo",
                periodo: "03/2026",
                vencimiento: "2026-03-25",
            },

            {
                mes: "Abril",
                periodo: "04/2026",
                vencimiento: "2026-04-24",
            },

            {
                mes: "Mayo",
                periodo: "05/2026",
                vencimiento: "2026-05-25",
            },

            {
                mes: "Junio",
                periodo: "06/2026",
                vencimiento: "2026-06-24",
            },

            {
                mes: "Julio",
                periodo: "07/2026",
                vencimiento: "2026-07-27",
            },

            {
                mes: "Agosto",
                periodo: "08/2026",
                vencimiento: "2026-08-24",
            },

            {
                mes: "Setiembre",
                periodo: "09/2026",
                vencimiento: "2026-09-25",
            },

            {
                mes: "Octubre",
                periodo: "10/2026",
                vencimiento: "2026-10-26",
            },

            {
                mes: "Noviembre",
                periodo: "11/2026",
                vencimiento: "2026-11-25",
            },

            {
                mes: "Diciembre",
                periodo: "12/2026",
                vencimiento: "2026-12-23",
            },
        ],
    },

    fonasaAnual: {
        disponibleDesde: "2026-02-09",

        vencimiento: "2026-04-30",

        descripcion:
        "Declaración anual FONASA Servicios Personales para dígitos 6, 7, 8 y 9.",
    },
};


export default vencimientosBPS2026;
