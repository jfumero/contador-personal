...
export default function App() {
  ...
  function guardarObligaciones(nuevasObligaciones) {
    ...
    localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify(nuevasObligaciones)
    );
  }
  ...
  function eliminarObligacion(id) {
    const confirma = confirm(
      "¿Seguro que querés eliminar esta obligación?"
    );

    if (!confirma) return;

    const nuevasObligaciones =
    obligaciones.filter((item) => item.id !== id);
  ...
  const estadoBPS =
  obtenerEstadoVencimiento(
    empresa.certificados.bps.hasta
  );

  const estadoDGI =
  obtenerEstadoVencimiento(
    empresa.certificados.dgi.hasta
  );
  ...
  return (
    ...
    <div>
    <p className="mini">
    Contador personal Uruguay
    </p>

    <h1>{empresa.nombre}</h1>

    <p className="datos-principales">
    RUT: {empresa.rut} · Empresa BPS:{" "}
    {empresa.empresaBPS}
    </p>
    ...
    <span className={estadoBPS.clase}>
    {estadoBPS.texto}
    </span>
    </div>
    ...
    <strong>Desde:</strong>{" "}
    {formatearFecha(
      empresa.certificados.bps.desde
    )}
    </p>
    ...
    <span className={estadoDGI.clase}>
    {estadoDGI.texto}
    </span>
    </div>
    ...
    <strong>Desde:</strong>{" "}
    {formatearFecha(
      empresa.certificados.dgi.desde
    )}
    </p>
    ...
    <p className="nota">
    Requiere clave. No guardar usuario ni
    ...
    <footer>
    <p>
    Las fechas tributarias deben verificarse
    siempre contra fuentes oficiales de DGI,
    BPS y FONASA.
    </p>
    </footer>
    </main>
